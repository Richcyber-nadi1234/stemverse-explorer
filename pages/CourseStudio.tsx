...
import { Save, Plus, Trash, FileText, Video, GripVertical, Sparkles, Bot, X, Loader2, Check, Calendar, Award, AlertCircle, Image as ImageIcon, Film, Wand2, Play, Link as LinkIcon } from 'lucide-react';
...
export const CourseStudio: React.FC = () => {
  ...
  const updateAssignmentConfig = (id: string, field: 'dueDate' | 'maxScore', val: any) => {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            { text: "Analyze this video. Please provide: 1. A concise summary. 2. Key learning points. 3. Three quiz questions with answers based on the content." }
          ]
        }
      });

      setMediaResult(response.text || "No analysis generated.");
    } catch (e) {
        console.error(e);
  ...
  const handleGenerateAI = async () => {
    ...
    try {
        ...
        ${aiConfig.type === 'quiz' ? `Generate ${aiConfig.quizCount} questions with ${aiConfig.quizDifficulty} difficulty.` : ''}
  ...
  const applyAIContent = () => {
    if (activeModuleId && generatedContent) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 relative pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={handleSave} className="flex items-center px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700">
      ...
      <div className="flex gap-4 border-b border-slate-200">
          <button 
            ...
            className={`pb-3 px-2 font-bold text-sm ${activeTab === 'curriculum' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500'}`}
          >
              Curriculum Builder
          </button>
          <button 
            onClick={() => setActiveTab('media')}
            className={`pb-3 px-2 font-bold text-sm flex items-center gap-2 ${activeTab === 'media' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500'}`}
          >
            ...
            <div className="space-y-4 animate-in fade-in">
                ...
                <div key={mod.id} className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col gap-4 group shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <input 
                        ...
                        className="flex-1 p-2 border-none focus:ring-0 font-medium text-slate-900 placeholder-slate-400 text-lg bg-white"
                        placeholder="Module Title"
                    />
                    <div className="flex gap-2">
                        <button 
                        onClick={() => openAIModal(mod.id)}
                        className="flex items-center px-3 py-1.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-lg text-xs font-bold hover:opacity-90 transition-opacity shadow-sm"
                    ...
                    <div className="ml-12 mr-4 space-y-3">
                    {/* Type Selector */}
                    ...
                    {mod.contentType !== 'live_class' && (
                        <textarea 
                            ...
                            placeholder={mod.contentType === 'assignment' ? "Enter assignment instructions here..." : "Module content (text, script, or quiz data)..."}
                        />
                    )}
                    </div>
                </div>
      ...
      {activeTab === 'media' && (
          <div className="space-y-6 animate-in fade-in">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                      <div className="bg-blue-100 p-2 rounded-lg text-blue-600"><Play className="w-5 h-5" /></div>
                      <div>
                          <h4 className="font-bold text-sm">Video Analysis</h4>
                          <p className="text-xs text-slate-500">Generate summaries & quizzes</p>
                      </div>
                  </button>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-8">
                  ...
                  <div className="flex-1 space-y-4">
                      <h3 className="font-bold text-lg text-slate-900 mb-2">
                          {mediaMode === 'veo' ? 'Generate Video from Image' : mediaMode === 'edit' ? 'Edit Image with AI' : 'Analyze Video'}
                      </h3>
                                ...
                                <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:bg-slate-50 transition-colors cursor-pointer relative">
                                    ...
                                    {mediaImage ? (
                                        <div className="flex items-center justify-center gap-2 text-green-600 font-medium">
                                            <Check className="w-5 h-5" /> {mediaImage.name}
                                        </div>
                                    ) : (
                          ...
                          <>
                             <div>
                                ...
                                <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:bg-slate-50 transition-colors cursor-pointer relative">
                                    ...
                                    {analysisVideo ? (
                                        <div className="flex items-center justify-center gap-2 text-green-600 font-medium">
                                            <Check className="w-5 h-5" /> {analysisVideo.name}
                                        </div>
                                    ) : (
                                        <div className="text-slate-400 text-sm">
                  ...
                  <div className="flex-1 bg-slate-50 rounded-lg border border-slate-200 flex items-center justify-center min-h-[300px] p-4 relative overflow-hidden">
                      {mediaLoading ? (
                          <div className="text-center text-slate-500">
                              ...
                              <p>{mediaMode === 'veo' ? 'Generating frames... (may take a minute)' : 'Processing...'}</p>
                          </div>
                      ) : mediaResult ? (
                          mediaMode === 'veo' ? (
                              ...
                              <div className="prose prose-sm max-w-none w-full h-full overflow-y-auto p-2">
                                  <h4 className="font-bold text-slate-900 mb-2">Analysis Result:</h4>
                                  <p className="whitespace-pre-wrap text-slate-700">{mediaResult}</p>
                              </div>
                          )
                      ) : (
              ...
              {!generatedContent ? (
                ...
                <div className="space-y-4 h-full flex flex-col">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => setGeneratedContent('')} className="text-xs text-indigo-600 hover:underline">Generate Different</button>
                  </div>
                  <textarea 
